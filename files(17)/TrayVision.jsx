...
const SEVERITY_COLORS = {
  'Ate fully': { bg: '#052e16', border: '#16a34a', text: '#4ade80', icon: '✅' },
  'Partially': { bg: '#431407', border: '#ea580c', text: '#fb923c', icon: '⚠️' },
  'Refused':   { bg: '#450a0a', border: '#dc2626', text: '#f87171', icon: '❌' },
}
...
export default function TrayVision({ patient, mealTime = 'lunch', onLogged }) {
  ...
  const s = {
    ...
    header: {
      ...
      fontFamily: 'var(--font-head)', fontSize: 15, fontWeight: 700, color: '#60a5fa'
    },
    badge: {
      background: '#1e3a5f', borderRadius: 6, padding: '2px 10px',
    ...
    analyzing: {
      textAlign: 'center', padding: '32px 0', color: '#60a5fa'
    },
    bar: { background: '#1e293b', borderRadius: 8, height: 8, marginTop: 4 },
    fill: (pct, color) => ({
  ...
  return (
    <div style={s.card}>
      <div style={s.header}>
        ...
        <span style={{ ...s.badge, background: '#1a2e1a', color: '#4ade80' }}>STOLEN: NeoPulse</span>
      </div>
          ...
          <div>
            ...
            {va.clinical_notes && (
              <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, padding: 12, marginBottom: 12 }}>
                <div style={{ color: '#64748b', fontSize: 11, fontWeight: 600, marginBottom: 4 }}>CLINICAL OBSERVATION</div>
                <div style={{ color: '#cbd5e1', fontSize: 13, lineHeight: 1.5 }}>{va.clinical_notes}</div>
              </div>
            )}
            ...
            {va.flags && va.flags.length > 0 && (
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                {va.flags.map(f => (
                  <span key={f} style={{ background: '#450a0a', border: '1px solid #dc2626', borderRadius: 6, padding: '3px 10px', fontSize: 11, color: '#f87171' }}>
                    ⚠ {f.replace(/_/g, ' ')}
                  </span>
                ))}
              </div>
            ...
            {result.dietitian_alert && (
              <div style={{ background: '#450a0a', border: '1px solid #dc2626', borderRadius: 8, padding: 12, marginBottom: 12 }}>
                <div style={{ color: '#f87171', fontWeight: 700, fontSize: 13 }}>🚨 {result.alert_message}</div>
              </div>
            )}
