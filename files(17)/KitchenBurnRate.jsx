...
const STATUS_STYLES = {
  CRITICAL: { bg: '#450a0a', border: '#dc2626', text: '#f87171', icon: '🔴', label: 'CRITICAL — Order Immediately' },
  LOW:      { bg: '#431407', border: '#ea580c', text: '#fb923c', icon: '🟠', label: 'LOW — Order Within 48h' },
  OK:       { bg: '#052e16', border: '#16a34a', text: '#4ade80', icon: '🟢', label: 'Adequate Stock' },
}
...
export default function KitchenBurnRate({ forecastDays = 3 }) {
  ...
  const s = {
    card: { background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: 12, padding: 20, marginTop: 16 },
    ...
    alertRow: (status) => {
      ...
      return { background: st.bg, border: `1px solid ${st.border}`, borderRadius: 8, padding: '10px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }
    },
    bar: (pct, status) => ({
    ...
    <div style={s.card}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={kpi.label} style={{ background: '#0f172a', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
